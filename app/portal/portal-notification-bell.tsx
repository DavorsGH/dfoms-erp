...
export default function PortalNotificationBell() {
  ...
  const refreshUnread = useCallback(async () => {
    try {
      const response = await fetch(`/api/portal/notifications?limit=1&offset=0`);
  ...
  const loadPage = useCallback(async (offset: number, append: boolean) => {
    ...
    try {
      const response = await fetch(
        `/api/portal/notifications?limit=${PAGE_SIZE}&offset=${offset}`,
      );
      const payload = (await response.json()) as {
        notifications?: LesseeNotificationRow[];
        unreadCount?: number;
        hasMore?: boolean;
        error?: string;
      };
  ...
  async function markRead(row: LesseeNotificationRow) {
    if (row.read_at) return;

    const response = await fetch(`/api/portal/notifications/${row.id}`, {
      method: "PATCH",
    });
    const payload = (await response.json()) as {
      notification?: LesseeNotificationRow;
      error?: string;
    };
  ...
  async function handleMarkAllRead() {
    setMarkingAll(true);
    setError(null);
    const response = await fetch("/api/portal/notifications/mark-all-read", {
      method: "PATCH",
    });
  ...
  async function handleDelete(row: LesseeNotificationRow) {
  ...
  async function handleClearAllRead() {
    ...
    setError(null);
    const response = await fetch(
      "/api/portal/notifications/clear-all-read",
      { method: "DELETE" },
    );
    const payload = (await response.json()) as { error?: string };
    ...
    <div ref={panelRef} className="relative">
      ...
      {open ? (
        ...
        >
          ...
          <NotificationBellPushPrompt persona="lessee" open={open} />
