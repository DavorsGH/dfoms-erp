...
export async function POST(request: Request) {
  ...
  const { data: existing, error: existingError } = await admin
    .from("leases")
    .select("lease_id, status, rent_amount_ghs")
  ...
  const proposedRentRaw = body.proposed_rent_amount_ghs;
  if (proposedRentRaw != null && proposedRentRaw !== "") {
    if (existing.status !== "active") {
      return NextResponse.json(
        { error: "Rent changes can only be proposed on active leases." },
        { status: 400 },
      );
    }
    const proposed = parseNonNegativeNumber(
      proposedRentRaw,
      "proposed_rent_amount_ghs",
      true,
    );
      ...
      return NextResponse.json(
        {
          error: !proposed.ok
            ? proposed.error
            : "proposed_rent_amount_ghs is required",
        },
        { status: 400 },
      );
    }
    const currentRent = Number(existing.rent_amount_ghs);
    if (proposed.value !== currentRent) {
      patch.pending_rent_amount_ghs = proposed.value;
      patch.rent_change_status = "pending_staff_approval";
    }
  }
