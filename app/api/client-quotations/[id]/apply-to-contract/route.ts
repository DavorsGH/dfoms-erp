...
import { applyQuotationToServiceContract } from "@/utils/client-quotations-api";
...
export async function POST(_request: Request, context: RouteContext) {
  ...
  const { contract, error } = await applyQuotationToServiceContract(
    supabase,
    auth.tenantId,
    id,
  );
  ...
  if (error || !contract) {
    return NextResponse.json(
      { error: error ?? "Unable to apply quotation to contract." },
      { status: 400 },
    );
  }
