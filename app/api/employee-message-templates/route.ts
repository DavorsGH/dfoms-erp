...
import {
  ...
  trimEmployeeMessageTemplateInput,
  validateEmployeeMessageTemplateInput,
  type EmployeeMessageTemplateInput,
...
export async function POST(request: Request) {
  ...
  const body = rawBody as EmployeeMessageTemplateInput;
  const validationError = validateEmployeeMessageTemplateInput(body);
  if (validationError) {
    return NextResponse.json({ error: validationError }, { status: 400 });
  }

  const trimmed = trimEmployeeMessageTemplateInput(body);
  const supabase = await getTenantSupabase();
  const {
    data: { user },
  ...
  return NextResponse.json({
    template: normalizeEmployeeMessageTemplateRow(
      data as EmployeeMessageTemplateRow,
    ),
  });
}
