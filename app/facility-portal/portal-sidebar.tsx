...
import { isFacilityPortalPathActive } from "./portal-nav-config";

type FacilityPortalSidebarProps = {
...
export default function FacilityPortalSidebar({
  links,
  mobile = false,
  onNavigate,
  onClose,
}: FacilityPortalSidebarProps) {
  ...
  return (
    ...
    >
      ...
      >
        {links.map((link) => {
          const active = isFacilityPortalPathActive(pathname, link.href);
          return (
            <Link
              key={link.id}
              href={link.href}
