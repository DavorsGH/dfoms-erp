...
import FacilityPortalSignOutButton from "./sign-out-button";

type FacilityPortalTopBarProps = {
  userLabel: string;
  onMenuToggle: () => void;
...
export default function FacilityPortalTopBar({
  userLabel,
  onMenuToggle,
  mobileNavOpen,
}: FacilityPortalTopBarProps) {
  return (
    <header className="flex min-h-14 shrink-0 items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 py-2 md:px-6">
      ...
      <div className="flex shrink-0 items-center gap-2 md:gap-3">
        ...
        <FacilityPortalSignOutButton variant="topbar" />
      </div>
    </header>
  );
}
