...
import {
  ...
  channelIncludesSms,
  LESSEE_MESSAGE_TEMPLATE_CHANNELS,
  formatChannelLabel,
  validateLesseeMessageTemplateInput,
  type LesseeMessageTemplateChannel,
  type LesseeMessageTemplateRow,
} from "@/utils/lessee-message-templates-types";

type LesseeMessageTemplatesProps = {
  tenantId: string;
  initialTemplates: LesseeMessageTemplateRow[];
  fetchError: string | null;
};

const emptyForm = {
  name: "",
  channel: "email" as LesseeMessageTemplateChannel,
  subject: "",
  body: "",
};
...
export default function LesseeMessageTemplates({
  tenantId,
  initialTemplates,
  fetchError,
}: LesseeMessageTemplatesProps) {
  ...
  async function refreshTemplates() {
    ...
    const response = await fetch(
      `/api/admin/lessee-message-templates?${params}`,
    );
    const payload = (await response.json()) as {
      templates?: LesseeMessageTemplateRow[];
      error?: string;
    };
  ...
  function openEditForm(row: LesseeMessageTemplateRow) {
    setEditingId(row.id);
    setForm({
      name: row.name,
      channel: row.channel,
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    const clientValidation = validateLesseeMessageTemplateInput(payload);
    if (clientValidation) {
    ...
    const response = await fetch(
      editingId
        ? `/api/admin/lessee-message-templates/${editingId}`
        : "/api/admin/lessee-message-templates",
      {
  ...
  async function handleDeactivate(row: LesseeMessageTemplateRow) {
    if (
      !window.confirm(
    ...
    const response = await fetch(
      `/api/admin/lessee-message-templates/${row.id}?tenant_id=${encodeURIComponent(tenantId)}`,
      { method: "DELETE" },
    );
    const result = (await response.json()) as { error?: string };
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          >
            ...
            {LESSEE_MESSAGE_TEMPLATE_CHANNELS.map((channel) => (
                  ...
                  onChange={(event) =>
                    setForm((current) => ({
                      ...
                      channel: event.target
                        .value as LesseeMessageTemplateChannel,
                    }))
                  }
                  className={inputClassName}
                >
