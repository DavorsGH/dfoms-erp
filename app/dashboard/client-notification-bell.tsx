...
export default function ClientNotificationBell() {
  ...
  const refreshUnread = useCallback(async () => {
    try {
      const response = await fetch(
        `/api/client-portal/notifications?limit=1&offset=0`,
      );
  ...
  const loadPage = useCallback(async (offset: number, append: boolean) => {
    ...
    try {
      const response = await fetch(
        `/api/client-portal/notifications?limit=${PAGE_SIZE}&offset=${offset}`,
      );
  ...
  async function markRead(row: ClientNotificationRow) {
    if (row.read_at) return;

    const response = await fetch(
      `/api/client-portal/notifications/${row.id}`,
      { method: "PATCH" },
    );
    const payload = (await response.json()) as {
  ...
  async function handleSelect(row: ClientNotificationRow) {
    const href = resolveClientNotificationHref(row);
    await markRead(row);
  ...
  async function handleMarkAllRead() {
    ...
    setError(null);
    const response = await fetch(
      "/api/client-portal/notifications/mark-all-read",
      { method: "PATCH" },
    );
  ...
  async function handleDelete(row: ClientNotificationRow) {
  ...
  async function handleClearAllRead() {
    ...
    setError(null);
    const response = await fetch(
      "/api/client-portal/notifications/clear-all-read",
      { method: "DELETE" },
    );
    const payload = (await response.json()) as { error?: string };
    ...
    <div ref={panelRef} className="relative">
      ...
      {open ? (
        <div
          ...
          className={NOTIFICATION_DROPDOWN_PANEL_CLASS}
        >
