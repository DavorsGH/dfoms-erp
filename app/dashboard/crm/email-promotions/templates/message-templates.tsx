...
const emptyForm = {
  name: "",
  template_type: "marketing" as MessageTemplateType,
...
function Badge({
  ...
  label: string;
  tone: "blue" | "violet" | "slate" | "emerald" | "amber";
}) {
...
export default function MessageTemplates({
  ...
  const filteredTemplates = useMemo(() => {
    return templates.filter((row) => {
      if (typeFilter && row.template_type !== typeFilter) return false;
      if (channelFilter && row.channel !== channelFilter) return false;
      return true;
    });
  }, [templates, typeFilter, channelFilter]);
  ...
  function openEditForm(row: MessageTemplateRow) {
    setEditingId(row.id);
    setForm({
      name: row.name,
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    const payload = {
      ...
      body_email: form.body_email,
      body_sms: form.body_sms,
      variables: parseVariablesInput(form.variablesText),
    };

    const clientValidation = validateMessageTemplateInput(payload);
    if (clientValidation) {
  ...
  async function handleDeactivate(row: MessageTemplateRow) {
    ...
    const response = await fetch(`/api/message-templates/${row.id}`, {
      method: "DELETE",
    });
    const result = (await response.json()) as { error?: string };
          ...
          <form onSubmit={(event) => void handleSubmit(event)} className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setForm((current) => ({
                      ...current,
                      template_type: event.target.value as MessageTemplateType,
                    }))
                  }
                  className={inputClassName}
                >
            ...
            <div>
              ...
              <input
                ...
                className={inputClassName}
                placeholder="customer_name, amount, invoice_no"
              />
              <p className="mt-1 text-xs text-slate-500">
                Comma-separated placeholder names used in the body (stored as a
                JSON array).
              </p>
            </div>
                ...
                filteredTemplates.map((row, index) => (
                  <tr key={row.id} className={getStripedRowClassName(index)}>
                    ...
                    <td className="px-4 py-3">
                      <Badge
                        label={formatTemplateTypeLabel(row.template_type)}
                        tone={
                          row.template_type === "marketing" ? "violet" : "blue"
                        }
                      />
                    </td>
                    <td className="px-4 py-3">
