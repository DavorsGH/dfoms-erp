/**
 * Apply scripts/212_landlords_signature_url.sql to production.
 */
// @ts-nocheck
...
async function main() {
  ...
  for (const connectionString of candidates) {
    ...
    try {
      ...
      console.log("Applied 212_landlords_signature_url.sql to production");
      console.log("landlords signature columns:", rows.map((r) => r.column_name));
      await client.end();
      return;
    } catch (error) {
      lastError = error;
