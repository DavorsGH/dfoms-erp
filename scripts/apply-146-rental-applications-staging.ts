/**
 * Apply scripts/146_rental_applications.sql to staging.
 * Usage: npx tsx scripts/apply-146-rental-applications-staging.ts
 *
...
async function main() {
  ...
  const sql = readFileSync(
    resolve(process.cwd(), "scripts/146_rental_applications.sql"),
    "utf8",
  );
  ...
  if (!client) {
    throw new Error(
      "Could not connect to staging DB. Apply scripts/146_rental_applications.sql in the Supabase SQL Editor on wieflwbfdmjtsdnwbfii.",
    );
  }

  try {
    await client.query(sql);
    console.log("Applied script 146 on staging");
    const { rows: tables } = await client.query(`
      SELECT table_name
      FROM information_schema.tables
      WHERE table_schema = 'public'
        AND table_name IN ('rental_applications', 'rental_application_links')
      ORDER BY table_name
    `);
    console.log("Tables:", tables);
    if (tables.length !== 2) {
      throw new Error("Expected both rental_applications tables");
    }
    console.log("PASS rental application tables present");
  } finally {
    await client.end();
  }
}
