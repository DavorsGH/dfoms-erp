/**
 * Apply scripts/279_atomic_payroll_lock_reopen.sql to staging.
 *
 *   npx tsx scripts/apply-279-atomic-payroll-lock-reopen-staging.ts
 *   npx tsx scripts/apply-279-atomic-payroll-lock-reopen-staging.ts --verify-only
 */
...
async function assertFunctions(client: pg.Client) {
  for (const fn of ["lock_payroll_period", "reopen_payroll_period"]) {
  ...
  for (const fn of ["_post_payroll_lock_finance", "_delete_payroll_lock_finance"]) {
    ...
    if (Number(rows[0]?.cnt ?? 0) < 1) {
      throw new Error(`Missing helper ${fn}`);
    }
    console.log(`OK: ${fn} present`);
  }
}
...
async function main() {
  ...
  try {
    console.log(`=== STAGING ${verifyOnly ? "VERIFY" : "APPLY"} 279 atomic payroll lock/reopen ===`);
    if (!verifyOnly) {
