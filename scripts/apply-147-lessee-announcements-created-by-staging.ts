/**
 * Apply scripts/147_lessee_announcements_created_by_dual_author.sql to staging.
 * Usage: npx tsx scripts/apply-147-lessee-announcements-created-by-staging.ts
 *
...
async function main() {
  ...
  const sql = readFileSync(
    resolve(
      process.cwd(),
      "scripts/147_lessee_announcements_created_by_dual_author.sql",
    ),
    "utf8",
  );
  ...
  if (!client) {
    throw new Error(
      "Could not connect to staging DB. Apply scripts/147_lessee_announcements_created_by_dual_author.sql in the Supabase SQL Editor on wieflwbfdmjtsdnwbfii.",
    );
  }

  try {
    await client.query(sql);
    console.log("Applied script 147 on staging");
    ...
    if (rows.length > 0) {
      throw new Error("FK lessee_announcements_created_by_fkey still present");
    }
    console.log("PASS: lessee_announcements_created_by_fkey dropped");
  } finally {
    await client.end();
  }
}
