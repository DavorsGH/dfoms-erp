...
function runUnitTests() {
  // Go-live January — value only in January; FULL_YEAR = December snapshot (0).
  ...
  assert.equal(janGoLive[FULL_YEAR_INDEX], 0, "Jan go-live: FULL_YEAR = December (0)");

  // Go-live July — zero Jan-Jun, value in July only; FULL_YEAR = December (0).
  ...
  assert.equal(julGoLive[FULL_YEAR_INDEX], 0, "Jul go-live: FULL_YEAR = December (0)");

  // Go-live December — value in December; FULL_YEAR = December (unchanged behaviour).
  const decGoLive = calculateInventoryOpeningEquityByMonth(
    {
  ...
  for (let i = 0; i < 11; i += 1) {
    assert.equal(decGoLive[i], 0, `Dec go-live: month index ${i} should be 0`);
  }
  assert.equal(decGoLive[11], 500, "Dec go-live: December should be 500");
  assert.equal(decGoLive[FULL_YEAR_INDEX], 500, "Dec go-live: FULL_YEAR = December (500)");
