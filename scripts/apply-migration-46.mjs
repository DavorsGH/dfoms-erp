...
async function main() {
  ...
  try {
    const script46 = readFileSync(
      resolve(process.cwd(), "scripts/46_cascade_delete_and_purchase_rpcs.sql"),
      "utf8",
    );

    await runSql(client, "46_cascade_delete_and_purchase_rpcs.sql", script46);
    ...
    const verification = await client.query(`
      SELECT
        ...
        ) AS raw_is_archived_exists,
        EXISTS (
          SELECT 1 FROM pg_proc WHERE proname = 'delete_raw_material_cascade'
        ) AS cascade_fn_exists;
    `);
