-- Script 72: Delete test tenants (Caanta Market place, Davors Facilities Test account)
-- and all their data, across every tenant_id-scoped table plus the
-- Davors-scoped crm_subscriptions/customers rows tied to them.
--
-- Target tenant IDs (verify names in tenants before running):
--   d45eac58-d793-41da-bbd2-2cb9ba09a892
--   a9ace34d-7cc0-4825-8994-9d2bdf1dbb9a
--
-- Run on staging first. Requires service_role / postgres (auth.users delete).
...
DECLARE
  ...
  v_tenant_ids uuid[] := ARRAY[
    'd45eac58-d793-41da-bbd2-2cb9ba09a892'::uuid,
    'a9ace34d-7cc0-4825-8994-9d2bdf1dbb9a'::uuid
  ];
  v_customer_ids text[];
  v_auth_uids uuid[];
  v_table text;
