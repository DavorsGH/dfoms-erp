/**
 * Production: scan tenants for BS inventory vs on-hand carrying value alignment.
 * Run: npx tsx scripts/scan-partial-stock-wac-mismatch-production.ts
 */
...
async function main() {
  ...
  const partialStockSpotChecks: Array<Record<string, unknown>> = [];
  ...
  for (const tenant of tenants ?? []) {
    ...
    for (const product of inv.finishedProducts) {
      ...
      partialStockSpotChecks.push({
        ...
        lifetime_overstatement: r2(lifetimeValue - bsValue),
  ...
  console.log("\n=== Partial-stock spot checks (production) ===");
  console.table(partialStockSpotChecks);

  console.log("\n=== Alignment mismatches (should be empty) ===");
  if (mismatches.length === 0) {
    console.log("PASS — all partial-stock products: BS value = carrying");
  } else {
