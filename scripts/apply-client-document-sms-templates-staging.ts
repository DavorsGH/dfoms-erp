/**
 * Refresh client-document transactional SMS templates for all tenants on staging
 * that already have Quotation Sent / Invoice Created / Receipt Issued rules.
 *
 * Usage: npx tsx scripts/apply-client-document-sms-templates-staging.ts
 */
...
const CAANTA_STAGING_TENANT_ID = "61e8e5d9-9cdb-4b8d-9e44-ed0acc23d87b";

const TEMPLATE_NAMES = buildClientDocumentTemplateSpecs().map((spec) => spec.name);

async function loadTemplateSmsBodies(
...
async function main() {
  ...
  const reportSamples = [
    ...
    { label: "Caanta", tenantId: CAANTA_STAGING_TENANT_ID },
