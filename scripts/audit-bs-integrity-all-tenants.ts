/**
 * Read-only: FY Balance Sheet integrity sweep across ALL tenants.
 *
 * Usage:
 *   npx tsx scripts/audit-bs-integrity-all-tenants.ts --env-file .env.local
 *   npx tsx scripts/audit-bs-integrity-all-tenants.ts --env-file .env.local.backup --allow-production
 */
...
function parseArgs() {
  ...
  return { envFile, allowProduction, investigate, year };
}
...
async function main() {
  const { envFile, allowProduction, investigate, year } = parseArgs();
  ...
  if (isProduction && !allowProduction) {
    throw new Error("Production URL detected — pass --allow-production to confirm");
  }
  if (!isProduction && !isStaging) {
    console.warn(`Warning: URL does not match known staging/production refs: ${url}`);
  }
  ...
  console.log(`\n=== FY${year} BS integrity sweep [${envLabel}] | ${tenants?.length ?? 0} tenants ===\n`);

  const results = [];
  for (const tenant of (tenants as TenantRow[]) ?? []) {
    process.stderr.write(`Auditing ${tenant.name}...\n`);
    results.push(await auditTenant(admin, tenant, year));
  }
  ...
  if (investigate && imbalanced.length > 0) {
